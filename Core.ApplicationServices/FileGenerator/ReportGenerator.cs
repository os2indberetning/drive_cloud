...
namespace Core.ApplicationServices.FileGenerator
{
    public class ReportGenerator : IReportGenerator
    {
        ...
        public void WriteRecordsToFileAndAlterReportStatus()
        {
            ...
            {
                _logger.LogError($"{GetType().Name}, WriteRecordsToFileAndAlterReportStatus(), Error when writing records to IND01-file", e);
                throw;
            }
            ...
            if (!success)
            {
                ...
                _logger.LogError($"{GetType().Name}, WriteRecordsToFileAndAlterReportStatus(), Error when writing records to IND01-file, reports status not updated to processed.");
                return;
            }
